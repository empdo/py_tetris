...
class Menu:
    ...
    def options(self):
        ...
        text_options = {"font_name": "Source Code pro", "font_size": 18, "color": (
            255, 255, 255, 255), "anchor_x": 'center', "anchor_y": 'center', "bold": True}
...
menu = Menu(resume=board.resume_game, restart=board.init_game,
            options=board.resume_game)
...
def on_key_press(symbol, modifiers):
    ...
    if (not board.is_paused):
        ...
        if symbol == key.UP:
            menu.current_option = (
                menu.current_option - 1) % (len(menu.options) - 1)
        elif symbol == key.DOWN:
            menu.current_option = (
                menu.current_option + 1) % (len(menu.options) - 1)
...
def on_draw():
    ...
    text_options = {"font_name": 'Karmatic Arcade', "font_size": 18, "color": (
        255, 255, 255, 255), "anchor_x": 'center', "anchor_y": 'center', "bold": True}
